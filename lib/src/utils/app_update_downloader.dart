...
class AppUpdateDownloader {
  ...
  /// 当前下载进度
  AppUpdateProgress _progress = AppUpdateProgress.unknown();
  ...
  Future<File> download({
    ...
    try {
      ...
      final error = e is AppUpdateError 
          ? e 
          : AppUpdateError.download(e);
  ...
  Future<void> pause() async {
    if (_status != AppUpdateStatus.downloading || _isPaused) {
      return;
    }
  ...
  Future<File> resume() async {
    if (_status != AppUpdateStatus.paused) {
      if (_status == AppUpdateStatus.downloading) {
        return _downloadCompleter!.future;
      }
      return download();
  ...
  Future<void> cancel() async {
    if (_status == AppUpdateStatus.canceled || 
    ...
    try {
      ...
      if (response.statusCode == 206) {
        ...
        if (contentRange != null && contentRange.contains('/')) {
          ...
          totalSize = resumeFrom + (response.contentLength ?? 0);
        }
      } else {
        // 普通响应
        totalSize = response.contentLength ?? expectedFileSize ?? 0;
      }
      
      // 更新进度初始状态
      _progress = AppUpdateProgress(
  ...
  Future<void> _finalizeDownload() async {
    ...
    try {
      ...
      _isCompleted = true;
      _updateStatus(AppUpdateStatus.downloaded);
